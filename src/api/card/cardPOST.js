...
module.exports = async (req, res) => {
  ...
  if (isEmpty(content) || isEmpty(deadline) || isEmpty(state)) {
    return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.CARD_BLANK));
  }

  try {
    const newCard = await cardDB.createCard(req.body);

    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.CREATE_CARD_SUCCESS, newCard));
  } catch (error) {
    console.log(error);
